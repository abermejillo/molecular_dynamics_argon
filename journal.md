...
For the data blocking method the conclusion was the following: If $`x_{\text{negative}}`$ is defined as the point where the data decreases for the first time, that is, the point where the derivative is negative for the first time, then the trimming point is taken as $`4 x_{\text{negative}}`$. This factor of $`4`$ was found after some manual tests. Here is an example of $`100`$ fits we obtain

![alt text](results/W5_test_data_blocking.png)

The average error between the fit and the data over this data set of size $`100`$ is

$`\text{Error} = 0.118 \pm 0.017`$

Since the standard deviation in the error is quite small, we can say that our fitting function is "reliable".
...
For simulations in which the only computed observable is the specific function, a typical distribution of the times taken by each function is
   - simulate: 66.1%
...
Solid $`(T=0.26, a=1.5474) \quad D=0.00128\pm 0.00005`$             |  Liquid $`(T=9.93, a=1.5471) \quad D=1.88\pm 0.15`$             |  Gas $`(T=0.71, a = 6.8\cdot 1.5471) \quad D=0.36\pm 0.01`$
:-------------------------:|:-------------------------:|:-------------------------:
